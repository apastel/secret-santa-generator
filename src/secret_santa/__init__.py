...
def write_pairing_pdfs(
    ...
    for giver, receiver in mapping.items():
        ...
        c.drawCentredString(width / 2, 60, link_text)
        # compute bounding box for clickable area
        text_width = c.stringWidth(link_text, "Helvetica-Oblique", 10)
        x_left = (width - text_width) / 2
        # create a small vertical padding around the text
        y_bottom = 60 - 2
        y_top = 60 + 10
        c.linkURL(repo_url, (x_left, y_bottom, x_left + text_width, y_top), relative=0)
